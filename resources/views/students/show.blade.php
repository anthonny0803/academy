<x-app-layout>
    ...
    <div class="py-8 sm:py-12">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Header Card --}}
            <div
                class="relative overflow-hidden bg-gradient-to-r from-emerald-600 via-emerald-600 to-teal-700 dark:from-emerald-800 dark:via-emerald-800 dark:to-teal-900 rounded-2xl shadow-xl mb-8">
                ...
                <div class="relative px-6 py-8 sm:px-8">
                    ...
                    <a href="{{ route('students.index') }}"
                        class="inline-flex items-center gap-2 text-emerald-200 hover:text-white transition-colors mb-6">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-6">
                        <div
                            class="w-20 h-20 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center text-white font-bold text-2xl shadow-xl border border-white/30">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-3">
                                @if ($student->is_active)
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-400/20 text-green-100 border border-green-400/30">
                                        <span class="w-1.5 h-1.5 bg-green-400 rounded-full mr-1.5"></span>
                                        Activo
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-400/20 text-yellow-100 border border-yellow-400/30">
                    ...
                    <div class="p-6">
                        <dl class="space-y-4">
                            <div>
                                ...
                                <dd class="mt-1 text-gray-900 dark:text-white font-medium">{{ $student->full_name }}
                                </dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Correo electrónico</dt>
                                <dd class="mt-1 text-gray-900 dark:text-white">{{ $student->email ?? 'Sin correo' }}
                                </dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Documento de identidad
                                </dt>
                                <dd class="mt-1 text-gray-900 dark:text-white">
                                    {{ $student->document_id ?? 'Sin documento' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Sexo</dt>
                                <dd class="mt-1 text-gray-900 dark:text-white">{{ $student->sex ?? 'No especificado' }}
                                </dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Fecha de nacimiento
                                </dt>
                                <dd class="mt-1 text-gray-900 dark:text-white">
                                    {{ $student->birth_date?->format('d/m/Y') ?? 'No registrada' }}
                                    @if ($student->age)
                                        <span class="text-gray-500 dark:text-gray-400">({{ $student->age }}
                                            años)</span>
                                    @endif
                                </dd>
                            </div>
                    ...
                    <div class="p-6">
                        <dl class="space-y-4">
                                ...
                                <dd class="mt-1">
                                    @if ($student->representative)
                                        <a href="{{ route('representatives.show', $student->representative) }}"
                                            ...
                                            <span
                                                class="w-2 h-2 rounded-full mr-2 {{ $student->representative->is_active ? 'bg-green-500' : 'bg-yellow-500' }}"></span>
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-gray-900 dark:text-white">
                                    {{ $student->created_at->format('d/m/Y') }}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                {{ $student->enrollments->count() }} inscripción(es) registrada(s)</p>
                        </div>
                            ...
                            <table class="w-full">
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach ($student->enrollments as $enrollment)
                                        @php
                                            $statusColor =
                                                $statusColors[$enrollment->status] ?? $statusColors['activo'];
                                        @endphp
            ...
            <div
                ...
                <div class="p-6 flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('students.reassign-representative.form', $student) }}"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-medium rounded-xl hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                        </svg>
    ...
    <div id="situationModal-{{ $student->id }}"
        class="modal hidden fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
        <div
            class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-2xl w-96 max-w-[90vw] border border-gray-200 dark:border-gray-700">
            <h3 class="text-lg font-bold mb-2 text-gray-900 dark:text-white">Cambiar Situación</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Estudiante: <span
                    class="font-medium text-emerald-600 dark:text-emerald-400">{{ $student->full_name }}</span>
            </p>
            <form action="{{ route('students.situation', $student) }}" method="POST">
                ...
                <div class="mb-4">
                    <label for="situation-{{ $student->id }}"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Situación</label>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="closeSituationModal('{{ $student->id }}')"
                        class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 transition-colors">
                        Cancelar
                    </button>
                    <button type="submit"
                        class="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-emerald-600 to-teal-600 rounded-xl hover:from-emerald-700 hover:to-teal-700 transition-colors">
                        Guardar
                    </button>
                </div>
            </form>
            ...
            document.querySelectorAll("[data-dropdown-enrollment]").forEach(function(btn) {
                btn.addEventListener("click", function(e) {
                    ...
                    clone.classList.add("dropdown-clone", "bg-white", "dark:bg-gray-800", "border",
                        "border-gray-200", "dark:border-gray-700", "rounded-xl", "shadow-xl");
