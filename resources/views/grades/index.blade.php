        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="relative overflow-hidden bg-gradient-to-r from-amber-500 via-amber-500 to-orange-600 dark:from-amber-700 dark:via-amber-700 dark:to-orange-800 rounded-2xl shadow-xl mb-8">
                ...
                <div class="relative px-6 py-8 sm:px-8">
                    {{-- Back link --}}
                    <a href="{{ route('grade-columns.index', $sectionSubjectTeacher) }}" class="inline-flex items-center gap-2 text-amber-100 hover:text-white transition-colors mb-4">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div id="saveStatus" class="hidden">
                            <span class="saving hidden inline-flex items-center gap-2 px-4 py-2 bg-yellow-500/20 backdrop-blur-sm text-white rounded-xl border border-yellow-400/30">
                            ...
                            <span class="saved hidden inline-flex items-center gap-2 px-4 py-2 bg-green-500/20 backdrop-blur-sm text-white rounded-xl border border-green-400/30">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                </svg>
                                Guardado
                            </span>
                            <span class="error hidden inline-flex items-center gap-2 px-4 py-2 bg-red-500/20 backdrop-blur-sm text-white rounded-xl border border-red-400/30">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        <div class="flex flex-wrap items-center gap-6 text-sm">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-white">{{ $minGrade }} - {{ $maxGrade }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-gray-500 dark:text-gray-400">Aprobación:</span>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-bold bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400">
                                    {{ $passingGrade }}
                                </span>
                            </div>
                        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-900/50">
                            <tr>
                                <th class="px-4 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider sticky left-0 bg-gray-50 dark:bg-gray-900/50 z-10">
                                    Código
                                </th>
                                <th class="px-4 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider sticky left-[80px] bg-gray-50 dark:bg-gray-900/50 z-10">
                                    Estudiante
                                </th>
                                @foreach ($gradeColumns as $column)
                                    <th class="px-3 py-4 text-center min-w-[110px]" title="{{ $column->observation }}">
                                        <div class="text-xs text-amber-600 dark:text-amber-400 font-bold">{{ number_format($column->weight, 0) }}%</div>
                                        <div class="text-xs font-semibold text-gray-700 dark:text-gray-300 mt-1">{{ $column->name }}</div>
                                    </th>
                                @endforeach
                                <th class="px-4 py-4 text-center text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider bg-amber-50 dark:bg-amber-900/20">
                                    Promedio
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($enrollments as $enrollment)
                                ...
                                @endphp
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors" data-enrollment="{{ $enrollment->id }}">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap sticky left-[80px] bg-white dark:bg-gray-800 z-10">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-center bg-amber-50 dark:bg-amber-900/20 average-cell"
                                        ...
                                        @if ($average !== null)
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold {{ $average >= $passingGrade ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' }}">
                                ...
                                <tr>
                                    <td colspan="{{ count($gradeColumns) + 3 }}" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 dark:text-gray-400 font-medium">No hay estudiantes inscritos en esta sección</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                        ...
                        @if ($enrollments->count() > 0)
                            <tfoot>
                                <tr class="bg-gray-50 dark:bg-gray-900/50">
                                    <td class="px-4 py-3 font-bold text-gray-900 dark:text-white sticky left-0 bg-gray-50 dark:bg-gray-900/50" colspan="2">
                                        Ponderación
                                    </td>
                                    @foreach ($gradeColumns as $column)
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            inputsArray.forEach((input, index) => {
                ...
                input.addEventListener('keydown', function(e) {
                    if (e.key === 'Enter' || e.key === 'Tab') {
                        ...
                        if (nextInput) {
                            ...
                            // This is the last input - blur to indicate completion
                            this.blur();
                        }
                    }
                });
                ...
                // Save on blur (for mouse clicks outside)
            ...
            function updateAverage(enrollmentId) {
                ...
                if (totalWeight > 0) {
                    ...
                    averageCell.innerHTML = `<span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold ${colorClasses}">${average.toFixed(2)}</span>`;
                } else {
